// Just comment/uncomment line 26/27 and watch the magic happen
...
// shall we use vectors or lists as containers?
// the sort function is overloaded to deal with both cases

typedef std::list<Student_info> Student_container;
// typedef std::vector<Student_info> Student_container;

int sort (std::list<Student_info>& s)
{
  s.sort(compare);
  return 0;
}

int sort (std::vector<Student_info>& s)
{
...
int main()
{
  ...
  Student_container students;	        // a container of structures
  ...
  Student_container::const_iterator iter = students.begin();
